    ...
    <div className="min-h-screen bg-[#0C1628] px-4 sm:px-6 py-8 sm:py-10">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-white">오답 노트</h1>
        ...
        {filtered.length === 0 && (
          <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-10 text-center">
            ...
            <Link
              href="/levels"
              className="inline-block mt-4 px-5 py-2 rounded-lg bg-amber-500 text-slate-900 text-xs font-bold hover:bg-amber-400 transition-colors"
            >
              문제 풀러 가기
            </Link>
          </div>
        ...
        <div className="space-y-3">
          {filtered.map((item) => {
            ...
            return (
              <div key={item.id} className="rounded-xl border border-slate-700 bg-slate-800/40 overflow-hidden">
                {/* Summary row */}
                <button
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-amber-400">Lv.{item.question.difficulty_level}</span>
                      <span className="text-xs text-slate-500">{topicLabel[item.question.topic] ?? item.question.topic}</span>
                      {item.hintsUsed > 0 && (
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <span className={[
                      ...
                      item.isCorrect ? 'text-emerald-400' : 'text-slate-400',
                    ].join(' ')}>
                      {accuracy}%
                    </span>
                  ...
                  {isExpanded && (
                    ...
                    >
                      <div className="px-4 pb-4 pt-1 border-t border-slate-700/50">
                        {/* Passage excerpt */}
                        <p className="text-xs text-slate-500 leading-relaxed mb-4 line-clamp-3" style={{ wordBreak: 'keep-all' }}>
                              ...
                              {item.submittedChain.map((sentenceId, i) => {
                                ...
                                return (
                                  <div
                                    key={`my-${i}`}
                                    className={[
                                      'flex items-start gap-1.5 rounded-lg px-2 py-1.5 border text-xs',
                                      isSlotCorrect
                                  ...
                                  >
                                    <span className={[
                                      'shrink-0 w-4 h-4 rounded text-[9px] font-bold flex items-center justify-center',
                                      isSlotCorrect ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400',
                                    ].join(' ')}>
                                      {i + 1}
                                    </span>
                            ...
                            <div className="space-y-1.5">
                              {item.question.correct_chain.map((sentenceId, i) => {
                                ...
                                return (
                                  <div
                                    key={`correct-${i}`}
                                    className="flex items-start gap-1.5 rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-2 py-1.5 text-xs text-slate-300"
                                  >
                                    <span className="shrink-0 w-4 h-4 rounded bg-emerald-500/20 text-emerald-400 text-[9px] font-bold flex items-center justify-center">
                                      {i + 1}
                                    </span>
                                    <span className="line-clamp-2 leading-relaxed">
                                      {sentence?.text ?? ''}
                        ...
                        {!item.isCorrect && (
                          <Link
                            ...
                            className="mt-3 flex items-center justify-center gap-2 px-4 py-2 rounded-lg border border-amber-500/30 bg-amber-500/10 text-amber-400 text-xs font-medium hover:bg-amber-500/20 transition-colors"
                          >
                            이 문제 다시 풀기
                          </Link>
                        )}
