...
- id는 알파벳(a, b, c, d, ...)을 사용하되, **sentences 배열 내 카드 순서를 무작위로 섞어주세요.** 정답 카드가 알파벳순으로 나열되면 안 됩니다.

### correct_chain (정답 순서)
- 정답 카드의 id를 **논리적 인과 순서**대로 배열. 반드시 ${slots}개.
- sentences의 순서와 무관하게, 인과적으로 올바른 순서여야 합니다.
- 예시: sentences가 [d, a, c, b, e]로 섞여 있어도 correct_chain은 ["c", "a", "d"] 처럼 논리 순서를 따릅니다.
...
function shuffleSentenceIds(q: Record<string, unknown>): Record<string, unknown> {
  ...
  const oldIds = sentences.map(s => s.id)
  const newIds = [...oldIds]
