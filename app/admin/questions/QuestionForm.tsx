    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      <div>
        ...
        <textarea
          ...
          rows={5}
          required
          className="w-full rounded-lg bg-slate-800 border border-slate-700 text-sm px-3 py-2 text-slate-200 resize-y"
        />
      </div>

      {/* Conclusion */}
      <div>
        <label className="block text-xs text-slate-400 mb-1">결론 (증명할 명제)</label>
        <textarea
          value={conclusion}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs text-slate-400">보기 카드</label>
          <button
            type="button"
            onClick={addSentence}
        ...
        <div className="space-y-2">
          {sentences.map((s, i) => (
            <div key={i} className="flex gap-2 items-start">
              <span className="shrink-0 w-7 h-9 flex items-center justify-center rounded bg-slate-700 text-xs font-mono text-slate-300">
                {s.id}
              </span>
              <input
                value={s.text}
                ...
                placeholder={`보기 ${s.id}`}
                required
                className="flex-1 rounded-lg bg-slate-800 border border-slate-700 text-sm px-3 py-2 text-slate-200"
              />
              {sentences.length > 2 && (
                <button
                  type="button"
      ...
      {/* Correct chain */}
      <div>
        <label className="block text-xs text-slate-400 mb-1">정답 체인 (쉼표 구분: a, b, c)</label>
        <input
          value={correctChain}
          onChange={(e) => setCorrectChain(e.target.value)}
          placeholder="a, b, c"
          required
          className="w-full rounded-lg bg-slate-800 border border-slate-700 text-sm px-3 py-2 text-slate-200 font-mono"
        />
      </div>

      {/* Hints */}
      <div>
        <label className="block text-xs text-slate-400 mb-2">힌트</label>
        <div className="space-y-2">
          {hints.map((h, i) => (
            <div key={i} className="flex gap-2 items-start">
              <span className="shrink-0 w-7 h-9 flex items-center justify-center rounded bg-slate-700 text-xs text-slate-300">
                {h.level}
              </span>
              <input
                value={h.text}
      ...
      <div className="flex gap-3">
        <button
          type="submit"
          disabled={saving}
          className="px-6 py-2 rounded-lg bg-amber-500 text-slate-900 text-sm font-semibold hover:bg-amber-400 transition-colors disabled:opacity-50"
        >
        ...
        <button
          ...
          className="px-6 py-2 rounded-lg border border-slate-700 text-slate-300 text-sm hover:bg-slate-800 transition-colors"
        >
          취소
        </button>
      </div>
